...
class CLS_pacman(object):
    def __init__(self, n, maze, x, y, flag):
        self.n = n
        self.x,self.y = x,y 
        self.flag = flag
        self.mem = []
        self.mem1 = []
        self.blList = maze.data
        return
    ...
    def clmove(self,grid):
        ...
        for df in (-1 , 0, +1):
            ...
            if not self.test(grid,flag1):
                continue
            canFlag +=1
            self.mem1.append((self.x,self.y,flag1))
        label = self.mem1.pop(0)
        self.x,self.y,self.flag = label
        self.x += SPEED_X[self.flag]
        self.y += SPEED_Y[self.flag]
        return self.x, self.y
    def manyanmove(self,scr,grid,bx,by):
        grid[by][bx]=2
        self.mem = [(bx,by)]
        while True :
            if len(self.mem) == 0 :
                break
            self.x, self.y = self.mem.pop(-1)
            for flag in (0,1,2,3):
...
maze = CLS_maze('maze03.txt',pic0,pic1,20,100,16,32)
pacman = CLS_pacman(16,maze,0,0,0)
aList =[[20,100]]
while True :
    maze.draw(screen)
    pacman.hsmove(maze.data)
